...
/**
 * Runs K-Means clustering to extract the top K dominant colors from an array of RGB pixels.
 */
function kMeans(pixels: [number, number, number][], k: number, maxIterations = 10): [number, number, number][] {
    if (pixels.length === 0) return [];
    if (pixels.length <= k) return pixels;

    // Initialize centroids by spacing them across the pixel array
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        // Recompute centroids as the mean of their cluster
        let centroidsChanged = false;
        const nextCentroids: [number, number, number][] = [];
        ...
        for (let i = 0; i < k; i++) {
            ...
            const nextCentroid: [number, number, number] = [meanR, meanG, meanB];

            if (
                nextCentroid[0] !== centroids[i][0] ||
                nextCentroid[1] !== centroids[i][1] ||
                nextCentroid[2] !== centroids[i][2]
            ) {
    ...
    // Sort centroids by luminance/brightness to ensure beautiful gradients
    centroids.sort((a, b) => {
...
/**
 * Extracts a palette of colors from an image URL using K-Means.
 */
async function getIconColors(iconURL: string | null): Promise<string[]> {
    ...
    try {
        ...
        for (let i = 0; i < data.length; i += info.channels) {
            // If the image has an alpha channel, ignore transparent pixels
        ...
        // Map centroids to Hex color strings
        return centroids.map(([r, g, b]) => 
            "#" + [r, g, b].map(c => c.toString(16).padStart(2, "0")).join("")
        );
    } catch (error) {
        console.error("[getIconColors] Erro ao extrair cores do ícone:", error);
        return FALLBACK_GRADIENT;
    }
}

/**
 * Fetches and caches color gradient for the guild icon.
 */
async function getColorsForGuild(guild: Guild): Promise<string[]> {
...
/**
 * Generates a server banner image with a dynamic gradient based on the guild icon
 * and overlays the masked circle guild icon.
 */
export async function criarImagemPainel(guild: Guild): Promise<AttachmentBuilder | null> {
    ...
    try {
        ...
        let circularIconBuffer: Buffer | undefined;
        ...
        if (iconURL) {
            try {
                ...
                console.error("[criarImagemPainel] Erro ao carregar ícone do servidor:", error);
            }
        }
        ...
        if (iconBuffer) {
            ...
            circularIconBuffer = await sharp(iconBuffer)
